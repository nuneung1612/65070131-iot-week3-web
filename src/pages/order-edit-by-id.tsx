...
export default function OrderEditById() {
  ...
  const { data: menu } = useSWR<Menu[]>('/menu'); // Fetch all available coffees
  ...
  const orderEditForm = useForm({
    ...
    validate: {
      id: (value) => (!value ? "กรุณาเลือกกาแฟ" : null),
      quan: (value) => (value <= 0 ? "จำนวนต้องมากกว่า 0" : null),
    },
  });
        ...
        {!!order && (
          <form onSubmit={orderEditForm.onSubmit(handleSubmit)} className="space-y-8">
            <Select
              ...
              {...orderEditForm.getInputProps("id")}
            />
            ...
            <NumberInput
              ...
              placeholder="จำนวน"
              min={1}
              {...orderEditForm.getInputProps("quan")}
            />
            ...
            <TextInput
              ...
              {...orderEditForm.getInputProps("detail")}
            />
